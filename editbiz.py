...
class EditBusinessForm:
    ...
    def load_data(self):
        ...
        self.cursor.execute("""
            SELECT biz_name, category, start_date, end_date, description, aliases, 
        ...
        if row:
            keys = ["biz_name", "category", "start_date", "end_date", 
                    "description", "aliases", "image_path", "map_link", "external_url"]

            for k, v in zip(keys, row):
                if k == "description":
                    self.entries[k].delete("1.0", "end")
                    self.entries[k].insert("1.0", v or "")
                else:
                    self.entries[k].delete(0, tk.END)
                    self.entries[k].insert(0, v or "")

        # Load Predecessor (this biz is the child)
        self.cursor.execute("""
        ...
        row = self.cursor.fetchone()
        if row:
            preceded_id, rel_type = row
            self.cursor.execute("SELECT biz_name, start_date, end_date FROM Biz WHERE biz_id = ?", (preceded_id,))
            p = self.cursor.fetchone()
            if p:
                label = f"{p[0]} ({p[1]}–{p[2]})"
                self.entries['preceded_by'].config(text=label)
                self.preceded_by_id_map[label] = preceded_id
                self.relationship_dropdowns["preceded_by"].set(rel_type)

        # Load Successor (this biz is the parent)
        self.cursor.execute("""
        ...
        row = self.cursor.fetchone()
        if row:
            succeeded_id, rel_type = row
            self.cursor.execute("SELECT biz_name, start_date, end_date FROM Biz WHERE biz_id = ?", (succeeded_id,))
            s = self.cursor.fetchone()
            if s:
                label = f"{s[0]} ({s[1]}–{s[2]})"
                self.entries['succeeded_by'].config(text=label)
                self.succeeded_by_id_map[label] = succeeded_id
    ...
    def save(self):
        try:
            ...
            for k, widget in self.entries.items():
                print(f"Processing field: {k}, widget type: {type(widget)}")
            ...
            # Parse date fields
            start_date, _ = parse_date_input(data["start_date"])
            end_date, _ = parse_date_input(data["end_date"]) if data.get("end_date") else ("", "")

            if self.biz_id:  # Update existing business
                self.cursor.execute("""
                    UPDATE Biz SET
                        biz_name = ?, category = ?, start_date = ?, end_date = ?, 
                        description = ?, aliases = ?, image_path = ?, map_link = ?, 
                        external_url = ?
                    WHERE biz_id = ?
                """, (
                    data["biz_name"], data["category"], start_date, end_date,
                    data["description"], data["aliases"], data["image_path"], 
                    data["map_link"], data["external_url"],     
                    self.biz_id
                ))
            else:  # Insert new business
                self.cursor.execute("""
                    INSERT INTO Biz (
                        biz_name, category, start_date, end_date, description,
                        aliases, image_path, map_link, external_url
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    data["biz_name"], data["category"], start_date, end_date,
                    data["description"], data["aliases"], data["image_path"], 
